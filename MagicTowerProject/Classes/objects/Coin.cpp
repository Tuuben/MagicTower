...
bool Coin::init()
{
    ...
    // Delay physics enable
    getPhysicsBody()->setContactTestBitmask(false);
    getPhysicsBody()->setDynamic(true);
    auto wait = DelayTime::create(0.1f);
    auto func = CallFunc::create([this](){ this->getPhysicsBody()->setContactTestBitmask(true); });
    runAction(Sequence::create(wait, func, NULL));
